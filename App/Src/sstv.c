...
UTILS_Status gen_flash_data(GD5F2GM7_Info_Struct *gd5f2gm7_obj){
  if(1){// if(sstv_info.sstv_mode->sstv_mode == SCT1){
    ...
    for (gd5_RA = 0; gd5_RA < sstv_info.sstv_mode->sstv_dma_line_cnt * sstv_info.sstv_mode->loop_num; gd5_RA++){//sstv_info.sstv_mode->sstv_dma_line_cnt * sstv_info.sstv_mode->loop_num
      if(gd5_RA/3 < 80){
        start = UTILS_GetSysTick();
        for(i=0;i<len;i++){
          temp = i/3;
          temp2 = temp/len_6;
          if(i == temp*3){
    ...
    HAL_Delay(1);
    // if(GD5F2GM7_ReadFromCache(gd5f2gm7_obj, data_address, test, 2112, UTILS_DMA) != UTILS_OK){
    //   printf("GD5F2GM7_ReadFromCache error\r\n");
    //   return UTILS_ERROR;
    // }
    ...
    // printf("line 10:%x\r\n", sstv_rx_buffer[1]);
    // for(uint16_t i = 0;i<1056;i+=2){
    //   printf("%x ", *((uint16_t*)test+i));
    // }
    // printf("\r\n");
...
UTILS_Status SSTV_Transmit(void){
  ...
  // gen_test_signal(sstv_tx_buffer, sstv_info.sstv_mode->sstv_dma_line_length);// gen_test_signal(tx_buffer, sstv_info.sstv_mode->sstv_dma_line_length);
...
void SSTV_TIM_Loop_Callback(void){
  ...
  if(sstv_info._sstv_fsm == SSTV_FSM_Loop){
    ...
    if(sstv_info._pulse_porch_index < sstv_info.sstv_mode->pulse_porch_num[sstv_info._loop_index]){
      ...
      // printf("b %d %x %x\r\n", sstv_info._loop_index, frqh, frql);
      test_point[18]++;
      test_point[16] = sstv_tim->Instance->CNT;
    ...
    printf("isbusyflag: %x\r\n", isbusy_flag);
    printf("%x\r\n", sstv_info.tx_buffer_ptr+16);//sstv_info.sstv_mode->sstv_dma_line_length/2
    printf("%x\r\n", sstv_info.tx_buffer_ptr+1);
    printf("%x\r\n", sstv_info.tx_buffer_ptr);
    printf("RA: %d\r\n", sstv_info._flash_RA);
    // printf("CNT = %d\t\n", test_point[16]);
