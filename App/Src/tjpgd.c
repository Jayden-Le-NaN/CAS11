...
/**
 * Allocate a memory block from the decompressor's memory pool.
 *
 * This function attempts to allocate a block of memory of the specified size
 * from the memory pool associated with the decompressor object. The size is
 * aligned to a word boundary to ensure proper memory alignment.
 *
 * @param jd Pointer to the decompressor object containing the memory pool.
 * @param nd Number of bytes to allocate from the memory pool.
 * @return Pointer to the allocated memory block, or NULL if the memory
 *         pool does not have enough space to fulfill the request.
 */
 ...
 * @brief 对块数据进行逆离散余弦变换
 *
 * @param src 输入块数据，重构量化后的系数，
 * @param dst 指针指向存放结果的区域
 *
 * The input block data is de-quantized and pre-scaled for Arai Algorithm.
 * The output block data is stored in byte array format in the destination.
 */
...
/**
 * @brief Load all blocks in the MCU into working buffer
 *
 * This function loads all blocks in the Minimum Coded Unit (MCU) into the working buffer.
 * The working buffer is used for de-quantization and inverse discrete cosine transform (IDCT).
 *
 * @param jd Pointer to the decompressor object
 *
 * @return JDR_OK if all blocks have been loaded successfully, otherwise JDR_ERR.
 */
...
//TODO: 输入输出函数
// loadMem(JDEC* jdec, uint8_t* buf, uint16_t len)
// {
